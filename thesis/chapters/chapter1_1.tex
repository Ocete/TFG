...
+
$$ M \Big( \alpha |u\rangle + \beta |w\rangle \Big) = \alpha M (|u\rangle) + \beta M (|w\rangle) \Big $$
...
A base $|v_1\rangle, \dotsc, |v_n\rangle$ is said to be \emph{orthonormal} if every vector is a unit vector and they are pairwise orthogonal

-> Gap

Definition 1: [TODO: format]
...
1.1) $\langle \cdot | \cdot \rangle$ is conjugate symmetric. That is, for all $u,v$ in $V$, $\langle u | v \rangle = \overline{\langle v | u \rangle}$.

1.2) $\langle \cdot | \cdot \rangle$ is linear on the second variable. That is, for all $u,v,w$ in $V$ and $\alpha, \beta$ in $\mathds{C}$:
