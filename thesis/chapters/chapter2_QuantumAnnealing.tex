...
In order to encode the max-cut problem as a QUBO we made use of a transformation: \emph{$x_i$ and $x_j$ should not be in the same set} translated into the penalty $P(x_i + x_j - 2 x_i x_j)$, which might be counternituive at first. Some similar transformations are encoded in table \ref{penalties-table}, 
...
% TODO: finish description of the table.
