...
"""
def solve_qubo_dwave(Q, n_genome_reads, num_reads=100, label='', annealing_time=20):
...
def solve_denovo_assembly(reads, self_bias, multilocation, repetition,
			...
			write_solutions=False, filename='results.txt', label='', annealing_time=20):
	...
	solutions, qpu_time, max_chain_length = solve_qubo_dwave(Q, num_reads=10000, n_genome_reads=len(reads),
																label=label, annealing_time=annealing_time)
...
def stress_test(_from=3, _to=15, use_QA=False):
	...
	for annealing_time in times:
		...
		print('{}\t| {}\t\t| {}\t\t\t| {:.6f}\t| {:.6f}\t| {}\t\t| {:.6f}'.format(
				annealing_time, valid_cycles, n_sol_reached, e_delta, best_energy, max_chain_length, total_time))
